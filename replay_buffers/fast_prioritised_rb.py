...
class FastPrioritisedExperienceReplayBuffer(abstract_replay_buffer.ReplayBuffer):
    def __init__(self, max_capacity: int, batch_size: int, eps: float, state_shape: tuple[int, ...]):
        ...
        self._container = torch.empty((max_capacity, state_size + 1 + 1 + 1 + state_size), dtype=torch.float32)
        self.eps = eps
        self._weights = torch.zeros(max_capacity, dtype=torch.float32)
        self._indices = torch.empty((batch_size,), dtype=torch.int64)

    def store(self, state: ndarray, action: int, reward: float, done: bool,
              new_state: ndarray):
        entry = torch.tensor((*state, action, reward, done, *new_state), dtype=torch.float32)
